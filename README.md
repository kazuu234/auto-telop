...
> **「壊れているため開けません」と出る場合：** 署名していないアプリのため、ダウンロード直後は macOS の検疫属性で起動がブロックされます。配布zipに同梱の **`初回セットアップ.command` をダブルクリック**すると自動で解除され、アプリが起動できるようになります（初回のみ）。
...
3. 同じフォルダにある **`初回セットアップ.command` をダブルクリック**する
4. ターミナルに `[OK] 完了しました！` と表示されたら、`AutoTelop.app` をダブルクリックで起動できます
